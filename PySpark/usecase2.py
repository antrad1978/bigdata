...
res = get_sum_grouped_by_date(sales, 'years', 'Profit', 'Order Date', 'MM/dd/yyyy', 'Total Profit')
res = res.filter(col("years") < 2017)
res.show()

revenues = res.select("Profit").rdd.map(lambda x: x[0]).collect()
